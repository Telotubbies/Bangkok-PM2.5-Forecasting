#!/usr/bin/env python3
"""
train_session_based.py
----------------------
Train STC-HGAT with Session-Based Enhancements

Combines:
- Fire features (24 total features)
- Session type embeddings (weekday/weekend/holiday/fire_season)
...
from src.models.stc_hgat_session import SessionSTCHGAT
...
from src.models.session_enhancements import get_session_type, get_hour_of_day
...
class SessionDataset(PM25SequenceDataset):
    """Extended dataset with session information"""
    
    def __init__(self, data, seq_len, horizons, timestamps):
        super().__init__(data, seq_len, horizons)
    ...
    def __getitem__(self, idx):
        ...
        seq_timestamps = self.timestamps[idx:idx + self.seq_len]
        
        # Get session type (based on first timestamp in sequence)
        session_type = get_session_type(np.array([seq_timestamps[0]]))[0]
        
        # Get hour of day for each timestep
        hours = get_hour_of_day(seq_timestamps)
        
        return X, y, session_type, hours


def main():
    parser = argparse.ArgumentParser(description='Train Session-Based STC-HGAT')
    parser.add_argument('--config', type=str, default='params.yaml')
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--batch-size', type=int, default=32)
    parser.add_argument('--start-date', type=str, default='2024-01-01')
    parser.add_argument('--end-date', type=str, default='2024-11-30')
    parser.add_argument('--use-fire', action='store_true', default=True)
    parser.add_argument('--use-session-types', action='store_true', default=True)
    parser.add_argument('--use-session-boundaries', action='store_true', default=True)
    parser.add_argument('--use-cross-window', action='store_true', default=True)
    args = parser.parse_args()
    
    print("=" * 60)
    print("🎯 Session-Based STC-HGAT Training")
    print("=" * 60)
    ...
    if args.use_fire:
        ...
        if len(fire_df) > 0:
            ...
            print(f"✅ Fire features ready: {fire_data.shape}")
    ...
    train_dataset = SessionDataset(train_data_norm, seq_len, horizons, train_timestamps)
    val_dataset = SessionDataset(val_data_norm, seq_len, horizons, val_timestamps)
    test_dataset = SessionDataset(test_data_norm, seq_len, horizons, test_timestamps)
    ...
    # Initialize model
    print("\n🤖 Initializing Session-Based Model:")
    print("=" * 60)
    print(f"Features: {num_features}")
    print(f"Session Types: {args.use_session_types}")
    print(f"Session Boundaries: {args.use_session_boundaries}")
    print(f"Cross-Window Attention: {args.use_cross_window}")
    
    model = SessionSTCHGAT(
        ...
        use_session_types=args.use_session_types,
        use_session_boundaries=args.use_session_boundaries,
        use_cross_window=args.use_cross_window,
    ).to(device)
    ...
    # Store previous window for cross-window attention
    prev_window_cache = {}
    ...
    for epoch in range(num_epochs):
        ...
        for batch_data in train_loader:
            X, y, session_types, hours = batch_data
            X, y = X.to(device), y.to(device)
            session_types = session_types.to(device)
            ...
            # Get previous window if using cross-window attention
            prev_window = None
            # For simplicity, we'll skip cross-window in first implementation
    ...
    model_path = save_dir / f'stc_hgat_session_{timestamp}.pt'
    ...
    # Summary
    print("\n" + "=" * 60)
    print("🎉 SESSION-BASED TRAINING COMPLETE")
    print("=" * 60)
    print(f"\n📊 Configuration:")
    print(f"  Features: {num_features}")
    print(f"  Session Types: {args.use_session_types}")
    print(f"  Session Boundaries: {args.use_session_boundaries}")
    print(f"  Cross-Window: {args.use_cross_window}")
